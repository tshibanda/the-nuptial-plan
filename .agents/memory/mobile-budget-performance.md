...
**How to apply:** Keep the persisted selected ID as the first query input; use the generated query options with `enabled: false` until an actual ID is available, and use a safe numeric fallback only for component prop typing.