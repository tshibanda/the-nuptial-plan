    ...
    for (const wedding of weddings) {
      const result = await client.query(
        `INSERT INTO weddings
         ...
         RETURNING id`,
        [OWNER_ID, wedding.names, wedding.partner1, wedding.partner2, wedding.date, wedding.venue, wedding.budget, wedding.guests, wedding.notes],
      );
    ...
    for (const [weddingIndex, weddingId] of weddingIds.entries()) {
      ...
      for (const [index, template] of vendorTemplates.entries()) {
        ...
        const vendor = await client.query(
          ...
          [weddingId, name, category, status, Math.round(amount * multiplier), Math.round(amount * multiplier * 0.3), contactName, `${contactName.toLowerCase().replaceAll(" ", ".")}@example.test`, "06 12 34 56 78", index === 0 ? "Contrat et dégustation suivis" : "Contact principal du dossier"],
        );
        const vendorId = vendor.rows[0].id;
        await client.query(
          `INSERT INTO contracts (wedding_id,vendor_id,vendor_name,status,total_amount,deposit_amount,signed_at,notes)
           VALUES ($1,$2,$3,$4,$5,$6,$7,$8)`,
          [weddingId, vendorId, name, status === "confirmed" ? "signed" : "pending", Math.round(amount * multiplier), Math.round(amount * multiplier * 0.3), status === "confirmed" ? isoDate(-120 + weddingIndex * 20) : null, "Version relue avec les mariés et archivée dans le dossier."],
        );
        await client.query(
          `INSERT INTO payments (wedding_id,vendor_id,vendor_name,description,amount,due_date,status,paid_at,notes)
           VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9)`,
          [weddingId, vendorId, name, "Acompte contractuel", Math.round(amount * multiplier * 0.3), isoDate(30 + index * 18), index % 2 === 0 ? "paid" : "pending", index % 2 === 0 ? isoDate(-40 + index * 4) : null, "Échéance suivie par la planner."],
        );
      }

      for (const [index, category] of categories.entries()) {
        const allocated = Math.round(weddings[weddingIndex].budget * (0.12 + (index % 3) * 0.04));
        await client.query(
