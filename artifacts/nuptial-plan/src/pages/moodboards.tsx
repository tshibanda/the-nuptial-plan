...
type Board = { id: string; title: string; description: string; imageUrl: string; accent: string };
const schema = z.object({ title: z.string().min(1), description: z.string().optional(), imageUrl: z.string().url('Ajoutez une URL d’image valide').or(z.literal('')), accent: z.string() });
...
export default function Moodboards() {
  ...
  const form = useForm<FormData>({ resolver: zodResolver(schema), defaultValues: { title: '', description: '', imageUrl: '', accent: '#C8A96E' } });
  useEffect(() => { try { setBoards(JSON.parse(localStorage.getItem(key) || '[]')); } catch { setBoards([]); } }, []);
  const save = (data: FormData) => {
    const next: Board[] = [...boards, { ...data, description: data.description ?? '', id: `${Date.now()}` }];
