...
router.post("/", async (req, res): Promise<void> => {
  ...
  const [category] = await db.insert(budgetCategoriesTable).values({ ...body.data, weddingId }).returning();
...
router.patch("/:id", async (req, res): Promise<void> => {
  ...
  if (!body.success) { res.status(400).json({ error: "Invalid input" }); return; }
  const [category] = await db.update(budgetCategoriesTable).set(body.data).where(and(eq(budgetCategoriesTable.id, id), eq(budgetCategoriesTable.weddingId, weddingId))).returning();
