  ...
  return (
    <>
      <ScrollView style={{ flex: 1, backgroundColor: colors.background }} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
        ...
        <Section title="Trésorerie" subtitle="Suivez les mois creux avant qu’ils ne vous surprennent." colors={colors}>
          <View style={styles.formGrid}>
            ...
            <Button label="Ajouter" onPress={addEntry} colors={colors} />
          </View>
...
function Section({ title, subtitle, children, colors }: any) { return <View style={[styles.section, { backgroundColor: colors.card, borderColor: colors.border }]}><Text style={[styles.sectionTitle, { color: colors.foreground, fontFamily: SERIF }]}>{title}</Text><Text style={[styles.small, { color: colors.mutedForeground, fontFamily: SANS }]}>{subtitle}</Text><View style={{ marginTop: 18 }}>{children}</View></View>; }
