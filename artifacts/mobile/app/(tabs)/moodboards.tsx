...
export default function MoodboardsScreen() {
  ...
  const copy = language === 'fr' ? {
    photoAccess: 'Accès aux photos', photoAccessBody: 'Autorisez l’accès à votre photothèque pour ajouter une inspiration.', incomplete: 'Moodboard incomplet', incompleteBody: 'Ajoutez un titre et une image ou un lien Pinterest, Instagram ou Canva.',
    feature: 'vos moodboards et inspirations', label: 'Moodboards & inspirations', eyebrow: 'INSPIRATION VISUELLE', body: 'Créez des univers visuels pour vos propositions, vos rendez-vous clients et vos mariages.', inspirations: 'Vos inspirations', saved: (count: number) => `${count} univers enregistré${count > 1 ? 's' : ''}`, library: 'Une bibliothèque visuelle à construire',
    closeForm: 'Fermer le formulaire', addBoard: 'Ajouter un moodboard', newBoard: 'Nouveau moodboard', chooseImage: 'Choisir une image', urlPlaceholder: 'Lien Pinterest, Instagram ou Canva (https://…)', linkPreview: 'Aperçu du lien utilisé automatiquement', linkHint: 'Le lien sera conservé avec l’inspiration et ouvert au toucher.', titlePlaceholder: 'Nom de l’univers *', notesPlaceholder: 'Intention, couleurs, notes…', accent: 'Couleur repère', chooseColor: 'Choisir la couleur', save: 'Enregistrer le moodboard', emptyTitle: 'Votre galerie est prête', emptyBody: 'Ajoutez une première inspiration pour donner une direction à vos futurs mariages.', deleteTitle: 'Supprimer ce moodboard ?',
  } : {
    photoAccess: 'Photo access', photoAccessBody: 'Allow access to your photo library to add inspiration.', incomplete: 'Incomplete moodboard', incompleteBody: 'Add a title and an image or a Pinterest, Instagram or Canva link.',
    feature: 'your moodboards and inspiration', label: 'Moodboards & inspiration', eyebrow: 'VISUAL INSPIRATION', body: 'Create visual worlds for your proposals, client meetings and weddings.', inspirations: 'Your inspiration', saved: (count: number) => `${count} saved board${count === 1 ? '' : 's'}`, library: 'A visual library waiting to be built',
    closeForm: 'Close form', addBoard: 'Add moodboard', newBoard: 'New moodboard', chooseImage: 'Choose an image', urlPlaceholder: 'Pinterest, Instagram or Canva link (https://…)', linkPreview: 'Link preview used automatically', linkHint: 'The link will be kept with the inspiration and opened when tapped.', titlePlaceholder: 'World name *', notesPlaceholder: 'Intent, colors, notes…', accent: 'Accent color', chooseColor: 'Choose color', save: 'Save moodboard', emptyTitle: 'Your gallery is ready', emptyBody: 'Add a first inspiration to set the direction for future weddings.', deleteTitle: 'Delete this moodboard?',
  };
  ...
  const chooseImage = async () => {
    ...
    if (!permission.granted) {
      Alert.alert(copy.photoAccess, copy.photoAccessBody);
      return;
    }
  ...
  const save = () => {
    if (!title.trim() || (!imageUri && !sourceUrl.trim())) {
      Alert.alert(copy.incomplete, copy.incompleteBody);
      return;
    }
        ...
        ListHeaderComponent={
          <>
            <LinearGradient colors={[colors.plumDark, colors.plum, colors.plumLight]} style={styles.hero}>
              <Text style={[styles.eyebrow, { color: colors.gold, fontFamily: SANS_MEDIUM }]}>{copy.eyebrow}</Text>
            ...
            <View style={styles.toolbar}>
              <View>
                <Text style={[styles.sectionTitle, { color: colors.foreground, fontFamily: SERIF }]}>{copy.inspirations}</Text>
                <Text style={[styles.sectionBody, { color: colors.mutedForeground, fontFamily: SANS }]}>{boards.length ? copy.saved(boards.length) : copy.library}</Text>
              </View>
              <TouchableOpacity accessibilityRole="button" accessibilityLabel={adding ? copy.closeForm : copy.addBoard} onPress={() => { if (adding) { setAdding(false); } else { requirePremium(() => setAdding(true)); } }} style={[styles.iconButton, { backgroundColor: colors.plum }]}>
            ...
            {adding && <View style={[styles.form, { backgroundColor: colors.card, borderColor: colors.border }]}>
              <Text style={[styles.formTitle, { color: colors.foreground, fontFamily: SERIF }]}>{copy.newBoard}</Text>
              <TouchableOpacity onPress={chooseImage} style={[styles.imagePicker, { borderColor: colors.border, backgroundColor: colors.background }]}>
                {imageUri ? <Image source={{ uri: imageUri }} style={styles.preview} /> : <><Feather name="image" size={24} color={colors.plum} /><Text style={[styles.pickerText, { color: colors.mutedForeground, fontFamily: SANS }]}>{copy.chooseImage}</Text></>}
              </TouchableOpacity>
              <TextInput value={sourceUrl} onChangeText={handleSourceUrlChange} placeholder={copy.urlPlaceholder} placeholderTextColor={colors.mutedForeground} autoCapitalize="none" keyboardType="url" style={[styles.input, { backgroundColor: colors.background, borderColor: colors.border, color: colors.foreground }]} />
              ...
              {sourceUrl ? <Text style={[styles.linkHint, { color: colors.mutedForeground, fontFamily: SANS }]}>{copy.linkHint}</Text> : null}
              <TextInput value={title} onChangeText={setTitle} placeholder={copy.titlePlaceholder} placeholderTextColor={colors.mutedForeground} style={[styles.input, { backgroundColor: colors.background, borderColor: colors.border, color: colors.foreground }]} />
              <TextInput value={description} onChangeText={setDescription} placeholder={copy.notesPlaceholder} placeholderTextColor={colors.mutedForeground} multiline style={[styles.input, styles.textarea, { backgroundColor: colors.background, borderColor: colors.border, color: colors.foreground }]} />
              <Text style={[styles.label, { color: colors.mutedForeground, fontFamily: SANS }]}>{copy.accent}</Text>
              <View style={styles.accentRow}>{accents.map((item) => <TouchableOpacity key={item} onPress={() => setAccent(item)} accessibilityLabel={`${copy.chooseColor} ${item}`} style={[styles.swatch, { backgroundColor: item }, accent === item && styles.swatchSelected]} />)}</View>
              <TouchableOpacity onPress={save} style={[styles.saveButton, { backgroundColor: colors.plum }]}><Text style={[styles.saveText, { fontFamily: SANS_SEMIBOLD }]}>{copy.save}</Text></TouchableOpacity>
            </View>}
          </>
        }
        ListEmptyComponent={<View style={[styles.empty, { backgroundColor: colors.card, borderColor: colors.border }]}><Feather name="image" size={28} color={colors.goldDim} /><Text style={[styles.emptyTitle, { color: colors.foreground, fontFamily: SERIF }]}>{copy.emptyTitle}</Text><Text style={[styles.emptyBody, { color: colors.mutedForeground, fontFamily: SANS }]}>{copy.emptyBody}</Text></View>}
