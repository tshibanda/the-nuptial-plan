...
type Board = { id: string; title: string; description: string; imageUri: string; accent: string };
...
export default function MoodboardsScreen() {
  ...
  const save = () => {
    if (!title.trim() || !imageUri) {
      Alert.alert('Moodboard incomplet', 'Ajoutez un titre et une image pour enregistrer ce moodboard.');
      return;
    }
    persist([{ id: `${Date.now()}`, title: title.trim(), description: description.trim(), imageUri, accent }, ...boards]);
    setTitle(''); setDescription(''); setImageUri(''); setAccent(accents[0]); setAdding(false);
  };
